                ...
                using (db temp = new db())
                {
                    ...
                    foreach (purchase_invoice_detail purchase_invoice_detail in Local_PurchaseInvoice.purchase_invoice_detail)
                    {
                        foreach (item_movement item_movement in purchase_invoice_detail.item_movement)
                        {
                            ...
                            item_movement.Update_ChildVales(purchase_invoice_detail.unit_cost, true, Local_PurchaseInvoice.trans_date);
                        }
                    }
                }
            }
                                ...
                                else
                                {
                                    if (Diff > 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            List<StockList> Items_InStockLIST = CurrentItems.getProducts_InStock(LocalDetail.app_location.id_branch,DateTime.Now,true).Where(x => x.LocationID == (int)LocalDetail.id_location && x.ProductID == LocalDetail.item.item_product.FirstOrDefault().id_item_product).ToList();
                ...
                using (db temp = new db())
                {
                    ...
                    foreach (sales_invoice_detail sales_invoice_detail in Local_SalesInvoice.sales_invoice_detail)
                    {
                        ...
                        if (Oldsales_invoice_detail == null)
                        {
                            ...
                            List<StockList> Items_InStockLIST = CurrentItems.getProducts_InStock(sales_invoice_detail.app_location.id_branch,DateTime.Now,false).Where(x => x.LocationID == (int)sales_invoice_detail.id_location && x.ProductID == sales_invoice_detail.item.item_product.FirstOrDefault().id_item_product).ToList();
