 select proj.name as Project, task.id_project_task, task.parent_id_project_task as ParentTask, item.name as  Item, item.code as ItemCode,
 task.code as TaskCode, task.item_description as Task, task.status,contacts.name as Contact,contacts.code as ContactCode,Contacts.gov_code as GovermentId,
sum(task.quantity_est) as QuantityEst, 
sum(exe.Quantity) as QuantityReal, 
task.unit_cost_est as CostEst, exe.unit_cost as CostReal, 
