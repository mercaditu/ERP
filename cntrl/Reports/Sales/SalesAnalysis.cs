...
select
	...
	sum(if(item_movement.id_purchase_invoice_detail > 0, item_movement.credit, null)) as Purchase,
        sum(if(item_movement.id_transfer_detail > 0, item_movement.credit, null)) as TransferIn,
        sum(if(item_movement.id_inventory_detail > 0, item_movement.credit, null)) as InventoryIn,
        sum(if(item_movement.id_execution_detail > 0, item_movement.credit, null)) as ProductionIn,
        
	sum(if(item_movement.id_sales_invoice_detail > 0, item_movement.debit, null)) as Sales,
        sum(if(item_movement.id_transfer_detail > 0, item_movement.debit, null)) as TransferOut,
        sum(if(item_movement.id_inventory_detail > 0, item_movement.debit,  null)) as InventoryOut,
        sum(if(item_movement.id_execution_detail > 0, item_movement.debit, null)) as ProductionOut,
