        ...
        public void ReArrange_ProductMovement()
        {
            ...
            foreach (app_location location in app_locationList)
            {
                List<item_movement> movement = item_movement
                    .Where(x => x.id_company == CurrentSession.Id_Company && x.id_location == location.id_location)
                    .OrderBy(x => x.trans_date).ToList();

                //List<item_movement> movement_Credit = movement
                //    .Where(x => x.credit > 0)
                //    .OrderBy(x => x.trans_date).ToList();

                //List<item_movement> movement_Debit = movement
                //    .Where(x => x.debit > 0)
                //    .OrderBy(x => x.trans_date).ToList();
                ...
                foreach (item_movement credit_movement in movement.Where(x => x.credit > 0))
                {
                    ...
                    foreach (item_movement debit_movement in movement.Where(x => x.debit > 0 && x.is_read == false))
                    {
                        ...
                        if (credit_movement.item_movement_value.Sum(x => x.unit_value) > 0)
                        {
                        ...
                        { new_debit_movement.sales_invoice_detail.unit_cost = new_debit_movement.item_movement_value.Sum(x => x.unit_value); }
