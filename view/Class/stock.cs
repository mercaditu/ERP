...
namespace Cognitivo.Class
{
    ...
    public class StockCalculations
    {
        ...
        public DataTable NewInventory_DateRange(DateTime TransDate)
        {
            string query = @"   select branch.name as BranchName,
                                ...
                                and (item_movement.id_purchase_invoice_detail > 0 or item_movement.id_execution_detail > 0 or item_movement.id_inventory_detail > 0)
                                group by item_movement.id_movement
                                ) as inv
