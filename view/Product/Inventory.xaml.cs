        ...
        private void BindItemMovement()
        {
            ...
            if (app_location != null && item_inventory != null)
            {
                if (app_location != null)
                {
                    ...
                    List< StockList> StockList = CurrentItems.getProducts_InStock_GroupBy(app_location.id_branch, item_inventory.trans_date,true).Where(x=>x.LocationID== app_location.id_location).ToList();
                    List< StockList> BatchList = CurrentItems.getProducts_InStock_GroupBy(app_location.id_branch, item_inventory.trans_date,false).Where(x => x.can_expire && x.LocationID == app_location.id_location).ToList();
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            if (CurrentSession.UserRole.is_master)
            {
                if (item_inventoryitem_inventory_detailViewSource.View.CurrentItem is item_inventory_detail item_inventory_detail)
                {
                    ...
                    if (item_movement != null)
                    {
                        item_movement.Update_ChildVales(Convert.ToDecimal(cost), true, item_inventory_detail.timestamp);
                    }
                }
            }
