...
namespace Cognitivo.Project.Development
{
    public partial class Logistics : Page
    {
        ...
        private CollectionViewSource projectViewSource, project_taskViewSource;

        private cntrl.Curd.PurchaseTender PurchaseTender;

        private List<Logistic> LogisticsList = new List<Logistic>();
        private List<Logistic> LogisticsListService = new List<Logistic>();
                ...
                if (chkqtyneeded.IsChecked == true)
                {
                    ...
                    var item_List_group_basic = (from IT in ProjectTaskDB.project_task
                                                 ...
                                                 select new
                                                 {
                                                     ...
                                                     buyqty = (last.Sum(x => x.quantity_est) != 0 ? last.Sum(x => x.quantity_est) : 0) - (last.Key.IM != null ? last.Key.IM.credit : 0 - (last.Key.IM != null ? last.Key.IM.debit : 0)),
                                                     item = last.Key.items
                                                 }).ToList();
